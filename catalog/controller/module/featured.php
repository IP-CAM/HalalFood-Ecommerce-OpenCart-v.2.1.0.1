...
class ControllerModuleFeatured extends Controller {
	public function index($setting) {
		...
		if (!empty($setting['rareitems'])) {
			$rateitems = array_slice($setting['rareitems'], 0, (int)$setting['limit']);
		...
		if(!empty($setting['pr'])) 
		{
			$arrivals = array_slice($setting['pr'], 0, (int)$setting['limit']);
